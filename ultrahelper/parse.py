...
def parse_model_extended(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    globals().update({c.__name__ : c for vals in REGISTRY.values() for c in vals})
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in base_modules:
            ...
            {Detect, WorldDetect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB, ImagePoolingAttn, v10Detect,*REGISTRY['head']}
        ):
            args.append([ch[x] for x in f])
            if m is Segment or m is YOLOESegment:
                args[2] = make_divisible(min(args[2], max_channels) * width, 8)
            if m in {Detect, YOLOEDetect, Segment, YOLOESegment, Pose, OBB,*REGISTRY['head']}:
                m.legacy = legacy
