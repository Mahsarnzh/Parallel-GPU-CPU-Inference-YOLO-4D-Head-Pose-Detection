...
# # ─── raw‐kpt decoding helper ───
# def decode_keypoints(raw_kpts: torch.Tensor,
#                      feat_stride: list,
#                      w_px: int,
#                      h_px: int,
#                      input_size: int = 640
#                     ) -> list[list[tuple]]:
#     """
#     raw_kpts: (1, 3*K, total_cells)
#     feat_stride: list of strides ([8,16,32])
#     returns: list of length total_cells, each a list of K (px,py,conf) tuples
#     """
#     raw = raw_kpts[0].cpu()                  # (3*K, total_cells)
#     K = raw.shape[0] // 3
#     N = raw.shape[1]
#     raw = raw.view(K, 3, N)                  # (K,3,N)

#     # build unified grid_xy & stride_map
#     grid_xys, stride_maps = [], []
#     for s in feat_stride:
#         ny = input_size // int(s)
#         nx = input_size // int(s)
...
#             indexing="ij"
#         )
#         gxy = torch.stack((gx, gy), dim=-1).view(-1,2)  # (ny*nx,2)
#         grid_xys.append(gxy)
#         stride_maps.append(torch.full((gxy.shape[0],), s))
#     grid_xy    = torch.cat(grid_xys,    dim=0)         # (total_cells,2)
#     stride_map = torch.cat(stride_maps, dim=0)         # (total_cells,)

#     all_kps = []
#     for i in range(N):
#         gx, gy = grid_xy[i].tolist()
#         s      = stride_map[i].item()
#         kps_px = []
#         for j in range(K):
#             rx, ry, rlog = raw[j,0,i], raw[j,1,i], raw[j,2,i]
#             conf = torch.sigmoid(rlog).item()
#             abs_x = (gx + rx.item()) * s
#             abs_y = (gy + ry.item()) * s
#             px    = (abs_x / input_size) * w_px
#             py    = (abs_y / input_size) * h_px
#             kps_px.append((px, py, conf))
#         all_kps.append(kps_px)
...
# # ─── worker #2: CPU post‑processor + drawing ───
# def _postprocessor_worker(result_q: Queue, img_q: Queue):
#     pp_model    = load_postprocessor()
#     CONF_THRESH = 0.9
#     INPUT_SIZE  = 640
...
#                     x2,y2 = int(x2n*w_px), int(y2n*h_px)
#                 else:
#                     x1,y1,x2,y2 = map(int, (x1n,y1n,x2n,y2n))
#                 cv2.rectangle(frame,(x1,y1),(x2,y2),(0,255,100),2)
#                 cv2.putText(frame,f"{conf:.2f}",(x1,y1-5),
#                             cv2.FONT_HERSHEY_SIMPLEX,0.5,(0,255,0),2)

#             # decode & draw all keypoints
#             all_kps = decode_keypoints(raw_kpts, feat_stride, w_px, h_px, INPUT_SIZE)
#             for kps_px in all_kps:
#                 draw_keypoints(frame, kps_px, CONF_THRESH)
#                 draw_connections(frame, kps_px, EDGES, CONF_THRESH)
