...
class ModifiedPose(Pose):
    ...
    # def forward(self, x):
    #     # hook lives here so it always fires, train or eval
    #     from contextlib import nullcontext
    #     from ..utils.monitor_4d import monitor_4d_ops
    #     ctx = monitor_4d_ops(self) if os.getenv("MONITOR_4D", "0") == "1" else nullcontext()
    #     with ctx:
    #         feats = self.forward_head(x)
    #         return self.forward_postprocessor(feats)
    def forward(self,x):
...
#     def forward(self, x):
#         from contextlib import nullcontext
#         debug = True                       # flip to False to silence
#         ctx = monitor_4d_ops(self) if debug else nullcontext()
#         with ctx:
#             return self.forward_head(x)    # no extra prints
